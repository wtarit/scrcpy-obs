...
static char *get_bin_dir(void)
{
	...
	char *data = obs_module_get_config_path(obs_current_module(), "bin");
	(void)data; /* not used - module data path preferred */
...
static char *get_adb_dir(obs_data_t *settings)
{
	const char *explicit = obs_data_get_string(settings, "adb_dir");
	if (explicit && *explicit)
		return bstrdup(explicit);
	const char *env = getenv("SCRCPY_OBS_ADB_DIR");
	if (env && *env)
		return bstrdup(env);
	return NULL;
}
...
static void start_scrcpy(struct scrcpy_src *ctx, obs_data_t *settings)
{
	...
	char *adb_dir = get_adb_dir(settings);
...
static obs_properties_t *src_get_properties(void *data)
{
	...
	obs_properties_add_text(props, "serial",
				obs_module_text("Serial"),
				OBS_TEXT_DEFAULT);
	...
	obs_properties_add_path(props, "adb_dir",
				obs_module_text("AdbDir"),
