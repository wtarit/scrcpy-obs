...
static char *first_adb_serial(void); /* defined in ADB section below */
...
/* Returns bstrdup'd serial of first usable device, or NULL. */
static char *first_adb_serial(void)
{
	char *adb = get_adb_exe();
	char *output = adb_devices_l(adb);
	bfree(adb);
